...
$('.oe_website_sale').each(function () {
    ...
    $(oe_website_sale).on('click', 'a.js_add_to_cart', function (ev) {
        var $q = $(".my_cart_quantity");
        var $add_to_cart = $(ev.currentTarget);
        var $notif = $(".add_to_cart_notify", $add_to_cart.parent());
        var $add_qty = $("#add_qty_input", $add_to_cart.parent());
        $q.parent().parent().removeClass("hidden");
        $notif.removeClass("hidden");
		var qty = parseInt($q.html());
		if (isNaN(qty)){
			qty = 0;
		}
		$q.html(qty+parseInt($add_qty.val())).hide().fadeIn(600);
		$notif.hide().fadeIn(600);
    });
    ...
    $(oe_website_sale).on('change', 'input.js_variant_change, select.js_variant_change', function (ev) {
        var $link = $(ev.currentTarget);
        var $product_id = $link.parent().find('input.product_id');
        var $parent = $link.parent().parent();
        var $price = $("#variant_price", $link.parent().parent());
        var $input = $("#add_qty_input", $price.parent().parent());
        if ($input.val() == undefined){
			$input = $("#add_qty_input");
		}
        var option = $link.find("option[value="+ev.currentTarget.value+"]")
        $input.val(option.data("moq_value"));
        $input.data("min", option.data("moq_value"));
        var price_list = $link.data("price_values"); 
        var product_id = false;
		product_id = ev.currentTarget.value;
        for (var k in price_list){
			if (price_list[k][0] == product_id){
				$price.html('$'+price_to_str(price_list[k][1])); 
			}
		}
